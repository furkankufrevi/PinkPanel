...
func (s *Service) GetByDomainID(domainID int64) (*Certificate, error) {
	...
	err := s.DB.QueryRow(`
		...
		&autoRenew, &cert.CreatedAt, &cert.UpdatedAt,
	)
...
func (s *Service) Install(domainID int64, certType, certPath, keyPath, chainPath, issuer, domainNames string, expiresAt time.Time) (*Certificate, error) {
	...
	now := time.Now().UTC()
	res, err := s.DB.Exec(`
		INSERT INTO ssl_certificates (domain_id, type, cert_path, key_path, chain_path, issuer, domains, issued_at, expires_at, auto_renew, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)
